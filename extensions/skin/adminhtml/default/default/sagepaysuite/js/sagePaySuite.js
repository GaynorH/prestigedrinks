...
gridRowListener = function(event){

	var vpstxid = Event.findElement(event, 'tr').down().next().next().next().next().innerHTML.replace(/ +?/g, '');
	window.parent.$('repeatpaymentcode').value = vpstxid;
	closeModalWindow();
...
loadActions = function(){
	...
	if($(document).body.hasClassName('sagepaysuite-adminhtml-repeatpayment-index')){
		varienGlobalEvents.attachEventHandler('gridRowClick', gridRowListener);
	}
