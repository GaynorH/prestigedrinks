...
class Ebizmarts_SagePaySuite_Model_Observer_Checkout extends Ebizmarts_SagePaySuite_Model_Observer {
    ...
    protected function _getLastOrderId() {
        ...
        if($orderId==0){
            //try from get param
            $orderId = (int) Mage::app()->getRequest()->getParam('o');
        }
    ...
    public function controllerOnePageSuccess($o) {
        ...
        $invoice_param = (int) Mage::app()->getRequest()->getParam('i');

        if($this->getSession()->getCreateInvoicePayment(true) || $invoice_param == 1) {
