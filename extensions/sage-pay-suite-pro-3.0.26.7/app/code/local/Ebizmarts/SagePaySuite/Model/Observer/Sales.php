    ...
    public function addButtonToOrderView($observer) {
        ...
        if ($block instanceof Mage_Adminhtml_Block_Sales_Order_View) {
            ...
            if ($sagePayData) {
                ...
                //DIRECTREFUND.
                if($sagePayData->getToken()) {
                    $block->addButton('sagepay_directrefund', array(
                        'label' => Mage::helper('sagepaysuite')->__('Sage Pay - Direct Refund'),
                        'onclick' => 'setLocation(\'' . $block->getUrl('adminhtml/spsDirectrefund/new', array('_secure' => true, 'trn_id' => $sagePayData->getId())) . '\')',
                        'class' => 'go'
                    ));
                }
            }
