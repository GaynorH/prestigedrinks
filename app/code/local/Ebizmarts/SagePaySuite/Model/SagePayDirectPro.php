...
class Ebizmarts_SagePaySuite_Model_SagePayDirectPro extends Ebizmarts_SagePaySuite_Model_Api_Payment {
    ...
    public function completePayPalTransaction(array $request, $quote) {
        ...
        if ($trnCurrency == 'store') {
            $pdata ['Amount'] = $this->formatAmount($quote->getGrandTotal()-$session_surcharge, $quote->getCurrencyCode());
        }
        else if ($trnCurrency == 'switcher') {
            $pdata ['Amount'] = $this->formatAmount($quote->getGrandTotal()-$session_surcharge, Mage::app()->getStore()->getCurrentCurrencyCode());
        }
        else {
            $pdata ['Amount'] = $this->formatAmount($quote->getBaseGrandTotal()-$session_surcharge, $quote->getQuoteCurrencyCode());
        }
    ...
    protected function _getPayPalRequest() {
        ...
        if ($rq) { //Button checkout
            ...
            if(Mage::getStoreConfig('payment/sagepaypaypal/shipping_method_selection') == 'estimate'){
                ...
                $basket = Mage::helper('sagepaysuite')->getSagePayBasket($quoteObj,$force_xml);
                if(!empty($basket)) {
                    if($basket[0] == "<") {
