...
?>
<?php
class Flint_Feefo_Model_Config_Product_Reviews extends Flint_Feefo_Model_Config_Abstract
{

    protected $_defaultPath = 'flint_feefo/product_reviews';
    ...
    public function getGoogleStarsActive( $store = null ) {
        return $this->getConfigData( $store, 'google_stars' );
    }

    public function getLogoTemplateReviews( $store = null ) {
        return $this->getConfigData( $store, 'logo_template_reviews' );
    }

    public function getCaching( $store = null ) {
        return $this->getConfigData( $store, 'caching' );
    }
