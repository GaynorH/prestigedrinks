...
?>
<?php
class Flint_Feefo_Model_Observer
{

    public function salesQuoteItemSetCustomAttribute( $observer ) {
        $quoteItem = $observer->getQuoteItem();
        $product = $observer->getProduct();
        $quoteItem->setFeefoDisableFeeds( $product->getFeefoDisableFeeds() );
        $quoteItem->setFeefoDisableFrontend( $product->getFeefoDisableFrontend() );
        $quoteItem->setFeefoBusinessCategory( $product->getFeefoBusinessCategory() );
    }
